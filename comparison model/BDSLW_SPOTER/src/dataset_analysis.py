...
class BdSLW60Analyzer:
    ...
    def analyze_dataset_structure(self):
        ...
        for class_folder in class_folders:
            ...
            for i, video_path in enumerate(
                videos[:3]
            ):  # Sample first 3 videos per class
                try:
        ...
        print(
            f"✅ Analysis complete: {analysis['total_videos']} videos across {analysis['total_classes']} classes"
        )
        return analysis
    ...
    def compare_with_spoter_dataset(self):
        ...
        bdsl_characteristics = {
            ...
            "avg_video_length": np.mean(
                self.analysis_results.get("video_durations", [0])
            ),
            "total_videos": self.analysis_results.get("total_videos", 0),
            "videos_per_class": self.analysis_results.get("total_videos", 0)
            / max(1, self.analysis_results.get("total_classes", 1)),
        }
    ...
    def generate_visualization_report(self, output_dir="processed_data"):
        ...
        if self.analysis_results.get("class_distribution"):
            classes = list(self.analysis_results["class_distribution"].keys())[
                :20
            ]  # Top 20 classes
